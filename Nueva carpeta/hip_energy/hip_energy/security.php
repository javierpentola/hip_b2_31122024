...
// // Asignar el ID del admin (eliminar o ajustar según la lógica de sesión)
// $admin_id = $_SESSION['admin_id'];

// Asignar un valor ficticio a $admin_id para pruebas
$admin_id = 1; // **Ajusta este valor según tus necesidades de prueba**
...
try {
    ...
    if (!$admin) {
        // Admin no encontrado, opcionalmente manejar este caso
        // Por ejemplo, podrías asignar valores predeterminados o simplemente continuar
        // session_destroy();
        // header('Location: login.php');
        // exit();
        // Para pruebas, podrías asignar valores predeterminados
        $admin = [
            'id' => $admin_id,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Bloquear un país (opcional)
    if (isset($_POST['block_country'])) {
        $country_code = $_POST['country_code'];
        $reason = $_POST['country_reason'] ?? 'Sin motivo';

        // Validar formato de código de país (dos letras)
        if (!preg_match('/^[A-Z]{2}$/i', $country_code)) {
            $error_message = "Formato de código de país inválido.";
        } else {
            try {
                // Verificar si el país ya está bloqueado
                $stmt = $pdo->prepare("SELECT * FROM blocked_countries WHERE country_code = :code");
                $stmt->execute(['code' => strtoupper($country_code)]);
                $existing = $stmt->fetch(PDO::FETCH_ASSOC);
                if ($existing) {
                    $error_message = "El país ya está bloqueado.";
                } else {
                    // Insertar en blocked_countries
                    $stmt = $pdo->prepare("INSERT INTO blocked_countries (country_code, reason) VALUES (:code, :reason)");
                    $stmt->execute([
                        'code' => strtoupper($country_code),
                        'reason' => $reason
                    ]);
                    $success_message = "País $country_code ha sido bloqueado exitosamente.";
                }
            } catch (PDOException $e) {
                $error_message = "Error al bloquear el país: " . $e->getMessage();
            }
        }
    }
}
...
try {
    $stmt = $pdo->prepare("SELECT a.id, a.username, a.last_login, a.is_logged_in, r.name as role_name, a.role_id FROM admins a LEFT JOIN roles r ON a.role_id = r.id");
    $stmt->execute();
...
// Obtener lista de países bloqueados
try {
    $stmt = $pdo->prepare("SELECT * FROM blocked_countries");
    $stmt->execute();
    $blocked_countries = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die("Error al obtener la lista de países bloqueados: " . $e->getMessage());
}

// Obtener lista de accesos para el mapa
try {
    // Obtener countryCode y contar accesos, excluyendo países bloqueados
    $stmt = $pdo->prepare("SELECT countryCode, COUNT(*) as count FROM access_logs WHERE timestamp >= DATE_SUB(NOW(), INTERVAL 30 DAY) AND countryCode NOT IN (SELECT country_code FROM blocked_countries) GROUP BY countryCode");
    $stmt->execute();
